...
static int __utils_ds_list_create_double(UTILS_DS_ID id, UTILS_LIST *list) {
    ...
    qmeta->curr_size = 0;
    qmeta->withCache = list->withCache;
    qmeta->cacheSize = list->cacheSize;
    ...
    // Create an array of poiners to store the pointers to list nodes
    if(list->withCache == TRUE) {
        qmeta->__cache = (__UTILS_LIST_CACHE **)malloc(sizeof(__UTILS_LIST_CACHE *) * list->cacheSize);
        for(int i = 0; i < list->cacheSize; i++) {
            qmeta->__cache[i] = NULL;
        }
    } 
