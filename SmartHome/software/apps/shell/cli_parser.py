...
class CliParser(object):
    ...
    def __init__(self, confFile): 
        ...
        self.parser_cmn_intf = self.parser_config['parser']['parser_cmn_intf']
        ...
        self.cli_apps_list = []
        self.cli_uid_list = []
        self.cli_uid_type_list = []

    def readCliFile(self, cliFile, parser_cmn_intf):
        ...
        with open(self.parser_out + names[-1].split('.', 1)[0]+'.h', "w+") as cliGenFile:
            self.writeCliGenFile(cliConf, cliGenFile, parser_cmn_intf)


    def writeCliGenFile(self, cliConf, cliGenFile, parser_cmn_intf):
        parser_cmn_intf.write("#include " + '\"' + cliGenFile.name.split('/')[-1]  + '\"')
        ...
        cliGenFile.write('#ifndef INCLUDE_SHELL_QUERY_H__\n')
        cliGenFile.write('#define INCLUDE_SHELL_QUERY_H__\n\n')

        # Get the name from the cliConf File
        appName = cliConf.find('app')
        uid = cliConf.find('uid')
        uid_type = cliConf.find('uid_type')
        if appName is None:
            # If app name is not present in the cliConf file, use the fileName itself
            cliGenFile.write("SHELL_APP_QUERY %s_query[] = \n" %(cliGenFile.name.split(".", 1)[0]))
        else:
            cliGenFile.write("SHELL_APP_QUERY %s_query[] = \n" %(cliConf.find('app').text))
            self.cli_apps_list.append(appName.text)
            self.cli_uid_list.append(uid.text)

            if(uid_type.text == 'static'):
                self.cli_uid_type_list.append("SHELL_APP_UID_STATIC")
            else:
                self.cli_uid_type_list.append("SHELL_APP_UID_DYNAMIC")

        cliGenFile.write("{\n")
        for _cli in cliConf.findall('cli'):
            cliGenFile.write("\t{\n")
            cliGenFile.write('\t\t .token = ' + _cli.find('token').text + ',\n')
            cliGenFile.write('\t\t .full_query = \"' + _cli.get('name') + '\",\n')
            cliGenFile.write("\t},\n")
        
        cliGenFile.write("\t{\n")
        cliGenFile.write("\t\t .token = 0xFFFF,\n")
        cliGenFile.write("\t},\n")
    ...
    def parse(self):
        ...
        # Create a common header file for easy inclusion of all auto-generated cli files
        with open(self.parser_cmn_intf, "w+") as parser_cmn_intf:
            parser_cmn_intf.write("/******************************************************************************\n")
            parser_cmn_intf.write("  Note : This is a Auto Generated File. DO NOT EDIT THIS FILE !!!\n")
            parser_cmn_intf.write("  FileName : %s\n" %(parser_cmn_intf.name))
            parser_cmn_intf.write("  Date : %s\n"   %(datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
            parser_cmn_intf.write("*******************************************************************************/\n\n")
            parser_cmn_intf.write("#ifndef INCLUDE_SHELL_INTF_H__\n")
            parser_cmn_intf.write("#define INCLUDE_SHELL_INTF_H__\n\n\n")
            for cli_file in self.parser_files:
                self.readCliFile(self.parser_cli + cli_file, parser_cmn_intf)
            
            parser_cmn_intf.write("\n\n\nSHELL_SUPPORTED_APP shell_supported_apps[] = {\n")
            for i in range(len(self.cli_apps_list)):
                parser_cmn_intf.write("\t{\n")
                parser_cmn_intf.write('\t\t .appName = \"' + self.cli_apps_list[i] + '\",\n')
                parser_cmn_intf.write('\t\t .uid = ' + self.cli_uid_list[i] + ',\n')
                parser_cmn_intf.write('\t\t .uid_type = ' + self.cli_uid_type_list[i] + ',\n')
                parser_cmn_intf.write('\t\t .appArray = ' + self.cli_apps_list[i] + '_query' + ',\n')
                parser_cmn_intf.write("\t},\n")
            
            parser_cmn_intf.write("\t{\n")
            parser_cmn_intf.write('\t\t .appName = ' + "NULL\n")
            parser_cmn_intf.write("\t},\n")
