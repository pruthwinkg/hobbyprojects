...
#define MAPI_MAX_SUB_MIDS       16 /* This is per MID */
#define MAPI_MAX_PORTS_PER_MID  8 /* This is per MID */
#define MAPI_GET_UID(mid)       ((mid & 0xFFFF0000) >> 16)
#define MAPI_GET_MID(mid)       (mid & 0x0000FFFF)
...
typedef struct {
    ...
    uint16_t *src_ports;
    int internal_fd;
    int *external_fds; // Supports multiple ports per MID
    ...
    void *adv_config; // Advanced configurations
    char *buffer; // This is used for Rx/Tx
} MAPI_INTERNAL_INFO;

extern MAPI_INTERNAL_INFO **mapi_internal_info_map;
extern boolean mapi_initialized;
extern boolean __mapi_usage_started;
